...
- Implementation: default config sets `split.method: time` with `time_col: issue_d` (configs/pytorch_default.yaml), and training calls the time split when configured (src/data/split.py, src/training/train_nn.py).
...
- Config: `configs/pytorch_default.yaml` uses `split.method: time` and `split.time_col: issue_d`.
