...
def train_from_config(cfg_path: str | Path, notes: Optional[str] = None):
    ...
    model_backend_raw = str(model_cfg.get("backend", "pytorch")).lower()
    if model_backend_raw not in {"pytorch", "h2o"}:
        raise ValueError(f"Unsupported model backend: {model_backend_raw}")
    model_backend = model_backend_raw

    model_filename_cfg = out_cfg.get("model_filename", "loan_default_model.pt")
    if model_backend == "h2o":
        base_name = Path(str(model_filename_cfg)).stem or "loan_default_model_h2o"
        model_filename = f"{base_name}.zip"
    else:
        model_filename = model_filename_cfg if str(model_filename_cfg).endswith(".pt") else f"{Path(str(model_filename_cfg)).stem}.pt"
    model_path = models_dir / model_filename
    logger.info("Model backend=%s | output path=%s", model_backend, model_path)

    # Optional class weights for BCE (PyTorch only)
    class_weight_cfg = training_cfg.get("class_weight") if model_backend == "pytorch" else None
    if class_weight_cfg is not None:
        cw_resolved = None
        if isinstance(class_weight_cfg, str) and class_weight_cfg.lower() == "auto":
    ...
    clear_wandb_hooks()
    if model_backend == "pytorch" and wandb_enabled and wandb_run is not None:
    ...
    logger.info("Random seed set to %d", int(split_cfg.get("random_state", 42)))
    ...
    y_prob = np.asarray(result.get("y_prob")) if isinstance(result, dict) else result
    ...
    if wandb_enabled and wandb_run is not None:
        try:
                    ...
                    try:
                        ...
                        if lb_path_str:
                            ...
                            if lb_df is not None and not lb_df.empty:
                                ...
                                wandb.log({"h2o_leaderboard": lb_table})
            ...
            try:
                ...
                layers_str = (
                    "-".join(str(x) for x in (layers_val or []))
                    if isinstance(layers_val, (list, tuple))
                    else (str(layers_val) if layers_val is not None else "")
                )
                leader_id = None
                leader_algo = None
                if isinstance(result, dict):
                    leader_id = result.get("leader_id") or (device_info.get("leader_id") if isinstance(device_info, dict) else None)
                    leader_algo = result.get("leader_algo") or (device_info.get("leader_algo") if isinstance(device_info, dict) else None)
                elif isinstance(device_info, dict):
                    leader_id = device_info.get("leader_id")
                    leader_algo = device_info.get("leader_algo")
                ...
                if template:
                    ...
                    if model_backend == "h2o":
                        leader_tok = (leader_id or "leader").replace(" ", "_")
                ...
                try:
                    ...
                    # Reuse context for tag rendering (includes backend/H2O metadata)
                    rendered = []
                    for t in tag_tmps:
                        try:
