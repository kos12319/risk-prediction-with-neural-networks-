...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run a PyTorch dry training experiment using temp output dirs (no artifacts persisted)",
    )
    parser.add_argument(
        "--config",
        type=str,
        default="configs/pytorch_default.yaml",
        help="Path to YAML config (PyTorch backend only)",
    )
    ...
    if backend not in {"", "pytorch"}:
        raise SystemExit("PyTorch dryrun requires model.backend to be 'pytorch' or omitted")
    ...
    with _prepare_transient_config(base_cfg_path) as dry_cfg_path:
        results = pytorch_train(dry_cfg_path)
